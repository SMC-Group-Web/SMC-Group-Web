      ...
      <Link href={`/servicios/${servicio.slug}`}>
        <article className="group relative overflow-hidden rounded-2xl">
          <div className="relative h-64 w-full overflow-hidden bg-slate-200">
            ...
            <div className="absolute inset-0" style={{ background: "linear-gradient(to top, rgba(10,22,40,0.88) 0%, rgba(10,22,40,0.1) 60%, transparent 100%)" }} />
            ...
            <motion.div
              ...
              transition={{ delay: visible ? 0.2 : 0, duration: 0.5, ease: [0.22, 1, 0.36, 1] }}
            >
              {servicio.category && (
              ...
              <h3 className="text-base font-bold leading-snug text-white">{servicio.title}</h3>
              <p className="mt-1.5 text-xs font-semibold text-white/60">Ver servicio →</p>
            </motion.div>
          </div>
        </article>
      </Link>
    ...
    <div ref={sectionRef} className="relative w-full">
      ...
      <div className="px-4 sm:px-6 md:hidden">
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          {services.map((s, i) => <MobileCard key={s.id} servicio={s} index={i} />)}
        </div>
      ...
      {/* ── DESKTOP: texto izquierda — foto derecha ── */}
      <div className="hidden md:flex" style={{ overflow: "clip" }}>

        {/* LEFT: lista scrollable */}
        <motion.div
        ...
        >
          ...
          {services.map((s, i) => {
            ...
            return (
              ...
              >
                <Link href={`/servicios/${s.slug}`} className="group block w-full">
                      ...
                      {s.features && s.features.length > 0 && (
                        ...
                        >
                          {s.features.slice(0, 4).map((f) => (
                            <li key={f.text} className="flex items-center gap-2 text-xs text-slate-500">
                              <div className="h-1 w-1 shrink-0 rounded-full" style={{ background: "var(--primary)" }} />
                              {f.text}
                            </li>
                          ))}
                        </motion.ul>
          ...
          >
            <div className="relative h-full w-full overflow-hidden bg-slate-900">
              <AnimatePresence mode="wait">
                ...
                >
                  {services[active]?.image?.url ? (
                    <Image
                      src={services[active].image!.url!}
                      alt={services[active].image?.alt || services[active].title}
                      fill
                      sizes="50vw"
                      quality={92}
                  ...
                  <div className="absolute inset-0" style={{ background: "linear-gradient(225deg, rgba(10,22,40,0.15) 0%, transparent 50%)" }} />
                  ...
                  {/* Progress bar lateral izquierdo */}
                  ...
                  <div className="absolute right-8 top-8 z-10 flex items-center gap-3">
                    ...
                    <span className="text-lg font-semibold text-white/40">{String(services.length).padStart(2, "0")}</span>
                  </div>
                </motion.div>
              </AnimatePresence>
            </div>
